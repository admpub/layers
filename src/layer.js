...
Class.pt.create = function(){
    ...
    switch(config.type){
        ...
        break;
        case 2:
            var content = config.content = conType ? config.content : [config.content||'about:blank', 'auto'];
...
Class.pt.move = function(){
    ...
    $(config.move).on('mousedown', function(M){
        ...
        if($(this).attr('move') === 'ok'){
			...
			var border = 0,borderWidth = '';
			if(conf.layero.get(0).currentStyle){
				borderWidth=conf.layero.get(0).currentStyle.borderWidth;
			}else if(window.getComputedStyle){
				borderWidth=window.getComputedStyle(conf.layero.get(0), null).getPropertyValue('border-top-width');
			}
			if(borderWidth){
				borderWidth=borderWidth.replace(/p[xt]$/i,'');
				border=parseFloat(borderWidth);
			}
			if(border==NaN)border = 0;
